...
class ParticipantResponse(ParticipantBase):
    ...
    checked_in: bool = Field(default=False, description="是否已入场")
...
class VoteStatus(BaseModel):
    ...
    remaining_changes: int = Field(default=0, description="剩余改票次数")
    can_vote: bool = Field(default=False, description="是否可以投票")
    can_change: bool = Field(default=False, description="是否可以改票")


class VoteResults(BaseModel):
    debate_id: str = Field(..., description="辩题ID")
